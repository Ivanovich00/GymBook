...
class PantallaBrazo extends StatefulWidget {
  const PantallaBrazo({Key? key}) : super(key: key);
  @override
  State<PantallaBrazo> createState() => _PantallaBrazoState();
}

class _PantallaBrazoState extends State<PantallaBrazo> {
                  ...
                  children: [
                    ...
                    StreamBuilder<QuerySnapshot>(
                      stream: (dropdownvalue.toString() == 'Ordenar de A a Z' || dropdownvalue.toString() == 'Ordenar de Z a A') 
                        ? db.collection('ejercicios').doc('categorias').collection('brazo').orderBy('nombre', descending: _isDescending).snapshots()
                        : db.collection('ejercicios').doc('categorias').collection('brazo').orderBy('timestamp', descending: _isDescending).snapshots(),
                      builder: (context, snapshot) {
                      _isConnected();
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
              ...
              onPressed: () {
                ...
                Navigator.pushReplacement(context, PageTransition(type: PageTransitionType.fade, child: PantallaBrazo(), duration: Duration(milliseconds: 250)));
              }),
          elevation: 0,
          title: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
                          if (_controllerNombre.text == '' || _controllerDescripcion.text == '') {
                            ...
                            if (_namePic.toString().contains('File')) {
                              final destination = 'ejercicios/brazo/${uid_final.id}/foto_${uid_final.id}';
                      ...
                      try {
                        ...
                        if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
                          if (_controllerNombre.text == '' || _controllerDescripcion.text == '') {
                            ...
                            if (_namePic.toString().contains('File')) {
                              final destination = 'ejercicios/brazo/${_uid}/foto_${_uid}';
                            ...
                            Navigator.push(context, MaterialPageRoute(builder: (context) => const PantallaBrazo()));
                          }
                        }
                      } catch (_) {
                        Get.snackbar(
