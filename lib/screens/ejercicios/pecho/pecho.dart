...
class PantallaPecho extends StatefulWidget {
  const PantallaPecho({Key? key}) : super(key: key);
  @override
  State<PantallaPecho> createState() => _PantallaPechoState();
}

class _PantallaPechoState extends State<PantallaPecho> {
                  ...
                  children: [
                    ...
                    StreamBuilder<QuerySnapshot>(
                      stream: (dropdownvalue.toString() == 'Ordenar de A a Z' || dropdownvalue.toString() == 'Ordenar de Z a A') 
                        ? db.collection('ejercicios').doc('categorias').collection('pecho').orderBy('nombre', descending: _isDescending).snapshots()
                        : db.collection('ejercicios').doc('categorias').collection('pecho').orderBy('timestamp', descending: _isDescending).snapshots(),
                      builder: (context, snapshot) {
                      _isConnected();
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
              ...
              onPressed: () {
                ...
                Navigator.pushReplacement(context, PageTransition(type: PageTransitionType.fade, child: PantallaPecho(), duration: Duration(milliseconds: 250)));
              }),
          elevation: 0,
          title: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
                          if (_controllerNombre.text == '' || _controllerDescripcion.text == '') {
                            ...
                            if (_namePic.toString().contains('File')) {
                              final destination = 'ejercicios/pecho/${uid_final.id}/foto_${uid_final.id}';
                      ...
                      try {
                        ...
                        if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
                          if (_controllerNombre.text == '' || _controllerDescripcion.text == '') {
                            ...
                            if (_namePic.toString().contains('File')) {
                              final destination = 'ejercicios/pecho/${_uid}/foto_${_uid}';
                            ...
                            Navigator.push(context, MaterialPageRoute(builder: (context) => const PantallaPecho()));
                          }
                        }
                      } catch (_) {
                        Get.snackbar(
